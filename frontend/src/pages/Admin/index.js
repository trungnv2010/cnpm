...
const Admin = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetRevenueStatisticsQuery({ period: periodStatistics });
        ...
        <div className="max-w-[calc(100%-1rem)] grid grid-cols-3 gap-x-6 gap-y-6 bg-gray-200 ">
              ...
              <div class="flex flex-row items-center mt-3 text-black w-full">
                ...
                <ul className="w-full">
                  {dataTopSelling?.map((product, index) => (
                    <li
                      key={product.id}
                      className="flex items-center justify-between w-full py-2 border-b last:border-b-0 "
                    >
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className="w-64 text-sm font-medium">
                            {product.name}
                          </p>
                          <p className="text-xs text-gray-500">{product.id}</p>
                        </div>
                      </div>
                      <div className="ml-16 text-sm font-medium">
                        {product.total_sold}
                      </div>
                    </li>
                  ))}
          ...
          <div class="  p-4 border border-gray-200 rounded-md shadow-sm bg-white col-span-5">
            <div class="flex justify-between items-center mb-4">
              ...
              <div class="relative">
                <select
                  onChange={(e) => handleChangePeriodStatistics(e)}
                  class="block appearance-none w-full bg-white border border-gray-300  py-2 px-4 pr-8 rounded leading-tight focus:outline-none focus:border-blue-500"
                >
