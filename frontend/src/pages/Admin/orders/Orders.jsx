...
const Orders=()=>{
    ...
    const handleClickOutsideSearchUser = (event) => {
        if (listUserDropDownRef.current && !listUserDropDownRef.current.contains(event.target)) {
            setListUser(false);
        }
    }
    ...
    //Lấy data khách hàng
    const {data:dataCustomerRaw}=useGetSearchCustomerQuery({query:debouncedSearchTerm})
            ...
            <div className="max-w-[calc(100%-1rem)] grid grid-cols-6 gap-x-6 gap-y-6">
                ...
                <div className="w-full col-span-6 p-4 mx-auto bg-white border border-gray-200 rounded-md shadow-sm ">
                    ...
                    <div className="relative w-full mb-4" ref={listUserDropDownRef}>
                        ...
                        />
                        {listUser&&(<ul className="absolute left-0 right-0 w-full mt-1 overflow-y-auto bg-white border shadow-lg max-h-60">
                            {dataCustomer.map((customer, index) => (
                                ...
                                >
                                    <span><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="text-blue-700 size-6">
                    ...
                    <div className="flex flex-col p-3 mt-3 text-center text-black">
                        {Object.keys(chosenUser).length === 0 && (
                            <div className="flex flex-col items-center">
                                ...
                                <div className="mt-2 text-gray-300">Chưa có thông tin khách hàng</div>
                            </div>
                        )}
                        ...
                        {Object.keys(chosenUser).length > 0 && (
                            <div className="grid grid-rows-1 gap-1">
                                <div className="flex items-center justify-start font-bold text-left ">
                                    {chosenUser.name} -
                                    <span className="p-2 font-thin text-left ">{chosenUser.phone}</span>
                                    <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                ...
                                <div className="max-w-md p-1 text-left mr-[50rem]">
                                    <div className="flex items-center justify-between">
                                        <h3 className="items-center text-sm font-semibold uppercase">địa chỉ giao hàng</h3>
                                        <a href="#" className="text-sm text-blue-500 hover:underline">Thay đổi</a>
                                    </div>
                                    <div className="text-base text-left ">{chosenUser.name} -
                                        <span>{chosenUser.phone}</span></div>
                                    
                                    <p className="mt-1 text-gray-700">
                                    {(chosenUser?.shipping_addresses?.find(address => address.is_default === 1))?.address || 'Không có địa chỉ mặc định'}
